// Tooling lives in its own module so that linters and their large
// dependency trees never appear in the daemon's go.mod. The versions are
// pinned here and run through `go run -modfile=tools/go.mod`, so CI
// installs nothing and every machine lints with identical rules.
module github.com/nugget/sparkwrangler/tools
